...
class TCResNet14(BaseModel):
    """
    TCResNet14 architecture:
    Variable number of repeated instances of TemporalBlock1 and TemporalBlock2.
    """
    def __init__(self, dataset_name='GoogleSpeechCommands', dataset_args={}, use_bias=True, pool=False, bn=False, do_nas=False, nas_config={}, learned_dil=[], learned_rf=[], learned_ch=[]):
        ...
        self.num_channels = self.dataset_args['num_channels']
        ...
        self.tcn = TempNet(
                num_inputs = self.num_channels[0], 
        ...
        if not learned_ch: 
            self.out = nn.Linear(
                    in_features = self.num_channels[-1]*self.ft_in,
                    out_features = self.dataset_args['output_size'],
                    bias = self.use_bias
                    )
        else:
            self.out = nn.Linear(
                    in_features = learned_ch[-1]*self.ft_in,
                    out_features = self.dataset_args['output_size'],
                    bias = self.use_bias
                    )
...
class TempNet(BaseModel):
    ...
    """
    def __init__(self, num_inputs, num_channels, kernel_size, dropout=0.5, use_bias=True, bn=False, 
        learned_dil=[], learned_rf=[], learned_ch=[], do_nas=False, nas_config={}):
        ...
        for i in range(num_levels):
            ...
            if not learned_dil:
                ...
                if (i % 2) != 0:
                    dilation_size.append(learned_dil[2*i - j])
                    dilation_size.append(learned_dil[2*i+1 - j])
                else:
            ...
            if not learned_rf:
                ...
                if not learned_dil:
                    k.append(learned_rf[2*i])
                    k.append(learned_rf[2*i+1])
                    dilation_size[0] = 1
                    dilation_size[1] = 1
                else:
                    if (i % 2) != 0:
                        k.append(ceil(learned_rf[2*i - l]/dilation_size[0]))
                        k.append(ceil(learned_rf[2*i+1 - l]/dilation_size[1]))
                    else:
                        k.append(ceil(original_rf[i]/2**i))
                        k.append(ceil(learned_rf[2*i - l]/dilation_size[0]))
                        l += 1
...
class TempBlock1(BaseModel):
    ...
    """
    def __init__(self, ch_in, ch_out, k_size, dil, dropout=0.2, block_idx=None, use_bias=True, bn=False, 
        do_nas=False, nas_config={}):
