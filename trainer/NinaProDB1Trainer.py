...
class NinaProDB1Trainer(BaseTrainer):
    """
    Trainer class
    """
    def __init__(self, model, criterion, metric_ftns, optimizer, config, device, data_loader, data_dir, batch_size, do_validation=True, lr_scheduler=None, len_epoch=None):
        super(NinaProDB1Trainer, self).__init__(model, criterion, metric_ftns, optimizer, config)
        self.config = config
    ...
    def cross_val(self, n_folds):
        ...
        for i in range(1, n_folds+1):
            ...
            print('Iteration {}/{}'.format(i, n_folds))
            ...
            res = self.test()
            acc[i] = res['accuracy']#.cpu()
            print("Subj {} : {}".format(i, acc[i]))
    ...
    def _train_epoch(self, epoch):
        ...
        if self.train_data_loader.shuffle is True:
            np.random.shuffle(self.train_data_loader.indexes)
